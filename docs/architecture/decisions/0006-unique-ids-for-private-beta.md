...
- LPA Reference nymbers should not be used in URLs on external-facing systems. For external-facing APIs or services (such as the 3rd party supporter API we plan) that need to make direct reference to an LPA, a local UUID associated with an LPA should be used instead.
